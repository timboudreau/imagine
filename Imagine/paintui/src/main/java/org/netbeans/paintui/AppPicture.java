...
class AppPicture extends PictureImplementation {
    ...
    public AppPicture(RepaintHandle handle, BufferedImage img) {
        this(handle,new Dimension(img.getWidth(),img.getHeight()));
        
        Graphics2D g = state.layers.get(0).getSurface().getGraphics();
...
*/
    ...
    private LayerImplementation createInitialLayer(Dimension d) {
        LayerImplementation impl = 
                LayerFactory.getDefault().createLayer (getDefaultLayerName(1), getMasterRepaintHandle(),
                d);
        return impl;
    }
...
//                fire();
    ...
    synchronized void setHibernated(boolean val, boolean immediate, Runnable run) {
        ...
        Dimension d = getSize();
        hibernated = val;
        LayerImplementation[] layers = (LayerImplementation[])
                state.layers.toArray(new LayerImplementation[state.layers.size()]);
    ...
    private static final class LayersState {
        ...
        void deepCopy(java.util.List <LayerImplementation>layers) {
            java.util.List<LayerImplementation> newLayers = new LinkedList<LayerImplementation>();
