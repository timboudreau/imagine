...
public class CircleInspector extends InspectorFactory<CircleWrapper> {
    ...
    public Component get(CircleWrapper obj, Lookup lookup, int item, int of) {
        ...
        NumberModel centerXModel = NumberModel.ofDouble(DOUBLE.withMaximum(10000D).withMinimum(-2000D), obj::centerX, num -> {
            obj.setCenterX(num);
            if (shape != null) {
                shape.changed();
            }
        ...
        NumberModel centerYModel = NumberModel.ofDouble(DOUBLE.withMaximum(10000D).withMinimum(-2000D), obj::centerY, num -> {
            obj.setCenterY(num);
            if (shape != null) {
                shape.changed();
            }
        ...
        NumberModel radiusXModel = NumberModel.ofDouble(DOUBLE_NON_NEGATIVE.withMaximum(10000D), obj::radius, num -> {
            obj.setRadius(num);
            if (shape != null) {
                shape.changed();
            }
