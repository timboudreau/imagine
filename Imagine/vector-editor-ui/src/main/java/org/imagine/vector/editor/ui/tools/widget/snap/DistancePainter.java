...
final class DistancePainter extends OneTypePainter {
    ...
    protected void requestRepaint(RepaintHandle handle) {
        if (xActive) {
            handle.repaintArea(arrowX1);
            if (hasXb) {
                handle.repaintArea(arrowX2);
            }
        }
        if (yActive) {
            handle.repaintArea(arrowY1);
            if (hasYb) {
                handle.repaintArea(arrowY2);
            }
        }
    }
    ...
    protected void paint(Graphics2D g, Zoom zoom, ShapeElement selected) {
        ...
        g.setPaint(s.indicatorLineColor(SnapKind.DISTANCE, selected));
        if (xActive) {
            g.draw(arrowX1);
        }
        if (yActive) {
    ...
    @Override
    protected boolean snapOne(SnapPoint<ShapeSnapPointEntry> pt, Zoom zoom, ShapeElement selection) {
