...
final class MultiWriter {
    ...
    public <C extends WritableByteChannel & SeekableByteChannel> void writeTo(C channel,
        ...
        int count = 0;
        for (PaintKey<?> pk : keysIterable) {
            IndexEntry en = new IndexEntry(pk.id(), channel.position(), count);
            try {
