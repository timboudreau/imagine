...
public final class PaintKeyIO {
    ...
    public static PaintKeyWriter<String> stringWriter(PaintKey<?> key) {
        PaintKeyStringWriter sw = new PaintKeyStringWriter();
        sw.writeInt(key.idBase().hashCode());
        sw.writeInt(key.hashCode());
        return sw;
    }

    public static PaintKeyWriter<List<ByteBuffer>> binaryWriter(PaintKey<?> key) {
        PaintKeyBinaryWriter sw = new PaintKeyBinaryWriter();
        sw.writeInt(key.idBase().hashCode());
        sw.writeInt(key.hashCode());
        return sw;
    }
    ...
    public static String writeAsString(PaintKey<?> key) throws IOException {
        PaintKeyStringWriter w = new PaintKeyStringWriter();
    ...
    public static void write(PaintKey<?> key, StringBuilder into) throws IOException {
        PaintKeyStringWriter w = new PaintKeyStringWriter(into);
        w.writeInt(key.idBase().hashCode());
        w.writeInt(key.hashCode());
        key.writeTo(w);
    }
    ...
    public static PaintKey<?> read(CharSequence seq, int position) throws IOException {
        PaintKeyStringReader reader = new PaintKeyStringReader(position, seq);
    ...
    public static <C extends ReadableByteChannel & SeekableByteChannel> PaintKey<?> read(C channel) throws IOException {
        PaintKeyBinaryReader<C> reader = new PaintKeyBinaryReader<>(channel);
    ...
    private static <K extends PaintKey<P>, P extends Paint> K readOne(Class<K> type, PaintKeyReader reader, int hash) throws IOException {
        IOFunction<? super PaintKeyReader, ? extends K> rdr = readerFunction(type);
    ...
    private static <K extends PaintKey<P>, P extends Paint>
            IOFunction<? super PaintKeyReader, ? extends K> readerFunction(Class<K> type) {
        Function<PaintKeyReader, K> result = Accessor.readerForKeyType(type);
        if (result != null) {
            return rdr -> {
